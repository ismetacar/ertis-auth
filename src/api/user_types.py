...
def init_user_types_api(app, settings):
    ...
    async def create_user_type(request, membership_id, *args, **kwargs):
        ...
        body = request.json
        resource = await app.user_type_service.create_user_type(body, request.ctx.user, membership_id)
    ...
    async def update_user_type(request, membership_id, user_type_id, *args, **kwargs):
        await ensure_membership_is_exists(app.db, membership_id, request.ctx.user)
        body = request.json
        resource = await app.user_type_service.update_user_type(membership_id, user_type_id, body, request.ctx.user)
        return response.json(json.loads(json.dumps(resource, default=bson_to_json)), 200)
